...
<body>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" style="width: 300px;">
                <select name="companyId" id="companyId" lay-filter="companyId" lay-verify="required">
                    ...
                    <option value="0" <?php if($data['companyId'] === '0') echo 'selected'; ?>>公用</option>
                    <?php foreach ($company as $val) { ?>
                    <option value="<?=$val['id']?>" <?php if($data['companyId'] == $val['id']) echo 'selected'; ?>><?=$val['name']?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            ...
            <div class="layui-input-block" style="width: 90%">
                <table class="layui-table">
                    ...
                    <?php
                    $data_Apps  = explode(',', $data['apps']);
                    $data_Menu  = explode(',', $data['menuId']);
                    foreach ($apps as $val) { 
                    ?>
                    <tr>
                        <td>
                            <input type="checkbox" name="apps[]" title="<?=$val['title']?>" value="<?=$val['id']?>" lay-skin="primary" <?php if (in_array($val['id'], $data_Apps)) echo 'checked'; ?>>
                        </td>
                        <td>
                            <?php foreach ($menus as $valmenu) { ?>
                            <?php if ($valmenu['appId'] == $val['id']) { ?>
                            <input type="checkbox" name="menuId[]" title="<?=$valmenu['title']?>" value="<?=$valmenu['id']?>" lay-skin="primary" <?php if (in_array($valmenu['id'], $data_Menu)) echo 'checked'; ?>>
                            <?php } } ?>
                        </td>
                    </tr>
                    <?php } ?>
    ...
    <script>
        ...
        form.on('submit(dosubmit)', function(data) {
            $.ajax({
                url: "<?= ez\core\Route::createUrl('editRole')?>",
                data: data.field,
                type: "post",
                dataType: "json",
